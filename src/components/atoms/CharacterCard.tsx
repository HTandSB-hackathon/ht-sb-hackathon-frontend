...
import { FaHeart, FaRegHeart, FaMapMarkerAlt, FaComments, FaClock } from 'react-icons/fa';
...
export const CharacterCard: React.FC<CharacterCardProps> = ({
  ...
  const getCityTheme = (city: string) => {
    const themes: Record<string, { color: string; emoji: string; gradient: string }> = {
      '須賀川市': { color: 'purple', emoji: '🍇', gradient: 'linear(to-r, purple.400, pink.400)' },
      '三春町': { color: 'pink', emoji: '🌸', gradient: 'linear(to-r, pink.400, rose.400)' },
      '中島村': { color: 'green', emoji: '🌾', gradient: 'linear(to-r, green.400, teal.400)' },
    };
    return themes[city] || { color: 'gray', emoji: '🏔️', gradient: 'linear(to-r, gray.400, gray.500)' };
  };
        ...
        <VStack spacing={spacing} align="stretch">
          ...
          <HStack spacing={4} align="start">
            <Box position="relative">
              <Avatar
                size={avatarSize}
                src={character.isLocked ? undefined : character.profileImage}
            ...
            <VStack align="start" spacing="1" flex="1">
              ...
              <Tag size="sm" colorScheme={cityTheme.color} variant="subtle">
                ...
                <TagLabel>{character.isLocked ? "未解放" : character.occupation}</TagLabel>
              </Tag>
